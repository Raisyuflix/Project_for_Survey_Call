...
FROM dstatuspenggunaan a with(nolock)
       INNER JOIN dkontak b with(nolock)
		...
		INNER JOIN  (select distinct txtkontakid from dstatuspenggunaan WITH (nolock)--SUMDAT UNMATCHED_ALFA
				where txtsumberdataid='186721')ds ON a.txtkontakid=ds.txtkontakid
